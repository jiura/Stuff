...
function send_cookie_data(button) {
    var jsonText = JSON.stringify(button.closest('form').serializeObject());
    var getUrl = window.location;
    var baseUrl = getUrl.protocol + "//" + getUrl.host + "/" + getUrl.pathname.split('/')[1];

    $('#result').text(JSON.stringify(button.closest('form').serializeObject()));
